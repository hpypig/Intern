...
func UpdateRedis() { // logic 层
    updatedDataChan := make(chan entities.UpdatedDataResponse, 10)
    go midware.GetUpdatedData(updatedDataChan)
    var account int
    for {
        updatedData := <-updatedDataChan
        account++
        log.Println("更新数：", account)
        ...
        for _,stock := range content.Stocks {
            ...
            res, err := redis.PostIDTitleMediaZSet(StockListKey, content.PublishTime, content.Id, content.Title, content.Media)  // 向 stock 索引 添加 score id
            if err != nil {
            ...
            res2, err := redis.PostIDTitleMediaZSet(StockListKey2, content.PublishTime, content.Id, content.Title, content.Media)
        ...
        if content.TxtType == 1 {
            res, err :=redis.PostIDTitleMediaZSet(redis.ImportantNewsListKey, content.PublishTime, content.Id, content.Title, content.Media)
            if err != nil {
        ...
        for _,column := range content.Columns {
            ...
            res, err := redis.PostIDTitleMediaZSet(ColumnListKey, content.PublishTime, content.Id, content.Title, content.Media)
            if err != nil {
