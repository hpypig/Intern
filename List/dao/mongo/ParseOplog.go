...
func ParseOplogAfter(lastTime int64, workChan chan <- entities.Oplog) (err error) {
    sess := session.Copy()
    ...
    for {
        for iter.Next(&oplog) {
            ...
            workChan <- oplog // 更新索引，并记录 Ts
            // 我想每发一个 oplog 记录一次lastTime，但假如oplog解析失败，中途退出，那 lastTime 就记录错误了，会漏掉资讯
            //fmt.Printf("parse-oplog.Ts: %d\n",oplog.Ts)
            lastTime = oplog.Ts>>32
            res, err := redis.SetLastTime(lastTime)
            if err != nil {
                log.Println("SetLastTime err: ", err)
            }
            fmt.Printf("in parse - set lastime: %d %s \n",lastTime, res)
            oplog = entities.Oplog{}
        }
        if err = iter.Err(); err != nil {
            return err // 为什么会出这个错误
