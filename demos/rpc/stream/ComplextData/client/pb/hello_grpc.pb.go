...
type GreeterClient interface {
	...
	// 请求更新的ImportantNews
	UpdatedNewsReplies(ctx context.Context, in *UpdatedNewsRequest, opts ...grpc.CallOption) (Greeter_UpdatedNewsRepliesClient, error)
}
...
func (c *greeterClient) UpdatedNewsReplies(ctx context.Context, in *UpdatedNewsRequest, opts ...grpc.CallOption) (Greeter_UpdatedNewsRepliesClient, error) {
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[1], "/pb.Greeter/UpdatedNewsReplies", opts...)
	if err != nil {
		return nil, err
	}
	x := &greeterUpdatedNewsRepliesClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type Greeter_UpdatedNewsRepliesClient interface {
	Recv() (*UpdatedNewsResponse, error)
	grpc.ClientStream
}

type greeterUpdatedNewsRepliesClient struct {
	grpc.ClientStream
}

func (x *greeterUpdatedNewsRepliesClient) Recv() (*UpdatedNewsResponse, error) {
...
type GreeterServer interface {
	...
	// 请求更新的ImportantNews
	UpdatedNewsReplies(*UpdatedNewsRequest, Greeter_UpdatedNewsRepliesServer) error
...
func (UnimplementedGreeterServer) UpdatedNewsReplies(*UpdatedNewsRequest, Greeter_UpdatedNewsRepliesServer) error {
	return status.Errorf(codes.Unimplemented, "method UpdatedNewsReplies not implemented")
}
...
func _Greeter_UpdatedNewsReplies_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(UpdatedNewsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(GreeterServer).UpdatedNewsReplies(m, &greeterUpdatedNewsRepliesServer{stream})
}

type Greeter_UpdatedNewsRepliesServer interface {
	Send(*UpdatedNewsResponse) error
	grpc.ServerStream
}

type greeterUpdatedNewsRepliesServer struct {
	grpc.ServerStream
}

func (x *greeterUpdatedNewsRepliesServer) Send(m *UpdatedNewsResponse) error {
	return x.ServerStream.SendMsg(m)
}
...
var Greeter_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "UpdatedNewsReplies",
			Handler:       _Greeter_UpdatedNewsReplies_Handler,
			ServerStreams: true,
		},
	},
